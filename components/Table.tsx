...
const TableComponent = ({ headers, fields, data, searchField,
    dateFilter, dateRange, setDateRange, detailsEndPoint }) => {
    ...
    const redirectToDetails = (_id) => {
        Router.push(`${detailsEndPoint}/${_id}`)
    }
    return (
        <div className='relative shadow-md sm:rounded-lg'>
            <div className='justify-between'>
                {
                    searchField ? <div className="p-4 display: inline-block">
            ...
            <Table hoverable={true} className="text-center">
                <Table.Head>
                    ...
                    {
                        detailsEndPoint ? <Table.HeadCell>
                            Acciones
                        </Table.HeadCell> : null
                    }
                </Table.Head>
                <Table.Body className="dark:bg-slate-800">
                    {
                        showData.map((data, index) => (
                            <Table.Row key={index}>
                                ...
                                {
                                    detailsEndPoint ? <Table.Cell>
                                        <div className='block justify-content:center'>
                                            <Button onClick={event => redirectToDetails(data._id)}>Detalle</Button>
                                        </div>
                                    </Table.Cell> : <p></p>
                                }
                            </Table.Row>
